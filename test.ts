...
Deno.test("simple read", async () => {
    ...
    expect(entries[0]).toEqual("2025-03-17 14:17:29 status installed libc-bin:amd64 2.36-9+deb12u10");
    expect(entries[9]).toEqual("2025-03-17 14:17:20 configure gettext:amd64 0.21-12 <none>");
});
...
Deno.test("filtering", async () => {
    ...
    // returned in reverse order
    expect(entries[0]).toEqual("2025-03-17 14:17:29 status installed libc-bin:amd64 2.36-9+deb12u10");
    expect(entries[6]).toEqual("2025-03-17 14:17:21 status unpacked gettext:amd64 0.21-12");
});
...
Deno.test("limiting", async () => {
    ...
    // returned in reverse order
    expect(entries[0]).toEqual("2025-03-17 14:17:29 status installed libc-bin:amd64 2.36-9+deb12u10");
    expect(entries[2]).toEqual("2025-03-17 14:17:27 trigproc libc-bin:amd64 2.36-9+deb12u10 <none>");
});
...
Deno.test("filter, limit, and paginate", async () => {
    ...
    // returned in reverse order
    expect(page1.entries[0]).toEqual("2025-03-17 14:17:29 status installed libc-bin:amd64 2.36-9+deb12u10");
    expect(page1.entries[2]).toEqual("2025-03-17 14:17:26 status installed man-db:amd64 2.11.2-2");

    // result indicates there is more
    expect(page1.cont).toBeDefined();

 
    const page2Request = new Request(`${baseUrl}/fodder/simple.log?cont=${page1.cont}`)
    ...
    const page3Request = new Request(`${baseUrl}/fodder/simple.log?cont=${page2.cont}`)
    ...
    // returned in reverse order
    expect(page3.entries[0]).toEqual("2025-03-17 14:17:21 status unpacked gettext:amd64 0.21-12");
